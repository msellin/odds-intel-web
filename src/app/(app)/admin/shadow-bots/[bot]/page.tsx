...
const ALLOWED: Record<string, { title: string; subtitle: string; detail: string }> = {
  ...
  bot_trigger_1x2_sharp_tight_v1: {
    ...
    subtitle: "P_sharp − 1/odds ≥ 2% AND odds ≤ 2.50 · pooled over Coolbet/Epicbet/Unibet · PAPER · MEASURING, not believing",
    detail:
      "SHARP-TIGHT-INSTRUMENT (2026-09-15). This is an INSTRUMENT, not a strategy: it exists to measure one hypothesis, and both research rounds that produced it believe the hypothesis is probably luck. WHY IT EXISTS: an exhaustive 70,200-cell sweep concluded no OWN sharp configuration works — but it could not have found this one. That grid swept a constant expected-ROI floor (P × odds − 1) while the live gate is a constant probability-difference floor (P − 1/odds); since roi_edge = prob_edge × odds, the latter is a CURVE in odds that no constant-floor cell can express. Swept on the right functional form it is the only survivor: n=225, ROI +17.07%, CI [+4.18,+29.95], no losing fold, OOS +23.40%. WHY WE DO NOT BELIEVE IT: it is a twelve-day effect (+0.99% on n=79 before 2026-09-02 vs +25.76% on n=146 after; Coolbet alone on a constant 37-day pool jumps the same way), and margin-corrected OWN-BOOK closing-line value on the same legs reads −5.4% to −7.6% beside those +19–42% ROIs, against ~−7.2% for a randomly chosen leg. The selection buys about 1.9pp of CLV — real, and nowhere near the 7–8% vig it must clear. CLV converges ~200× faster than ROI, so believe the CLV. PROMOTION IS PRE-COMMITTED: margin-corrected own-book CLV > 0 with CI excluding zero at n ≥ 300. ROI may NEVER promote it at any value — per-bet return sd ≈ 1.3 means confirming a true +3% ROI needs ~15,600 settled bets. Retire if that CLV is below −2% at n ≥ 300. Pre-registration: dev/active/own-sharp-tight-preregistration.md.",
  },
  ...
  bot_team_total_paper_shadow_v1: {
    ...
    detail:
      "USE-COLLECTED-MARKETS. Fires on team_total_{home,away}_<line> markets for upcoming fixtures: fair value is the de-vigged two-way Pinnacle team-total line, and it records the best price among the books we can reach (Epicbet, Betano, Unibet) when the edge clears 0%. EUR 10 nominal, settled from the FINAL SCORE — so unlike corners there is no stats-coverage gap and nothing can go unsettled. A market we have collected all along and never modelled. PAPER ONLY (never in PLACEABLE_BOTS). No backtest number: this config has never been replayed, so judge it on forward CLV and ROI. Note the venue reality (EPICBET-AS-A-VENUE, 2026-09-11): Epicbet quotes the full team-total ladder. Coolbet and Unibet-Site offer team totals too, but until CB-UB-1H-TT-COLUMNS (2026-09-11) our parsers dropped them. They now fill the Now CB / Now UB columns — Unibet-Site usually has one line per team. The bot still picks only among Epicbet/Betano/Unibet, so those columns are for comparing prices, not a venue the bot uses.",
  },
