...
export function FootprintControl({ state, now }: { state: ControlState; now: number }) {
  return (
    <ToastProvider>
...
function FootprintRow() {
  ...
  return (
    <section id="coolbet-footprint" aria-labelledby="footprint-title" className="scroll-mt-20 rounded-xl border border-border bg-card/40 px-4 py-3">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
        <div className="min-w-0 space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {state.fleet.error && <span className="text-amber-300">State unreadable ({state.fleet.error}) · </span>}
            {f?.daemons_paused && <>Paused{f.daemons_paused_reason ? `: “${f.daemons_paused_reason}”` : ""} · </>}
